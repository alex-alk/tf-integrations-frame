...
class OdeonApiService extends AbstractApiService
{
    ...
    private function getCityByAirportCodeMap(): array
    {
        ...
        $options['body'] = json_encode($requestArr);
        $options['headers'] = ['Content-Type' => 'application/json'];

        $httpClient = HttpClient::create();
        $responseDepObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $options);
        $responseDep = json_decode($responseDepObj->getContent(), true);
    ...
    public function apiGetAvailabilityDates(AvailabilityDatesFilter $filter): AvailabilityDatesCollection
    {
        ...
        if ($json === null) {
            ...
            // $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, $options, $responseDepObj->getContent(), $responseDepObj->getStatusCode());
            ...
            $availabilityDatesCollection = ResponseConverter::convertToCollection(json_decode($json, true), AvailabilityDatesCollection::class);
        }      
    ...
    public function apiGetCountries(): CountryCollection
    {
        ...
        $responseObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $options);
        $responseData = json_decode($responseObj->getContent(false), true);
        ...
        foreach ($countriesResponse as $value) {
            ...
            if (!isset($map[trim($value['CountryLName'])])) {
                $country->Code = $value['CountryLName'];
            } else {
    ...
    private function cacheToken(): string
    {
        ...
        $responseObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $optionsLogin);
        $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, $optionsLogin, $responseObj->getContent(false), $responseObj->getStatusCode());
        $response = json_decode($responseObj->getContent(false), true);
    ...
    private function cacheBookingToken(): string
    {
        ...
        $this->showRequest(HttpClient::METHOD_POST, $url, $optionsLogin, $responseObj->getContent(false), $responseObj->getStatusCode());
        $response = json_decode($responseObj->getContent(false), true);
    ...
    public function apiGetHotels(?HotelsFilter $filter = null): HotelCollection
    {
        ...
        if ($hotelsJson === null) {
            ...
            $hotelsArray = json_decode($hotelsJson, true);
    ...
    public function apiGetHotelDetails(HotelDetailsFilter $filter): Hotel
    {
        ...
        $responseObj = $httpClient->request(HttpClient::METHOD_GET, $url);
        $this->showRequest(HttpClient::METHOD_GET, $url, [], $responseObj->getContent(false), $responseObj->getStatusCode());
        $responseUrl = json_decode($responseObj->getContent(false), true);
        ...
        $responseInfo = $httpClient->request(HttpClient::METHOD_GET, $hotelUrl);
        $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, [], $responseInfo->getContent(false), $responseInfo->getStatusCode());
        if ($responseInfo->getStatusCode() == 404) {
            return $details;
        }

        $response = json_decode($responseInfo->getContent(), true);
    ...
    public function apiGetOffers(AvailabilityFilter $filter): AvailabilityCollection
    {
        ...
        if (!empty($filter->cityId)) {
            ...
            $cityIdArr = [];
            $citiesFromRegion->each(function(City $city) use (&$cityIdArr) { $cityIdArr[] = $city->Id; });
            $citiesList = implode(',', $cityIdArr);
        }

        $hotelId = $filter->hotelId;

        $httpClient = HttpClient::create(['max_duration' => 120]);
        ...
        do {
            ...
            $options['body'] = json_encode($requestArr);
            $responseObject = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $options);
            ...
            if ($i % 10 === 0) {
                foreach ($batch as $request) {
                    ...
                    $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, $optionsResp, $responseObj->getContent(), $responseObj->getStatusCode());
                    $responseData = json_decode($responseObj->getContent(), true);
                    ...
                    if ($responseData['Error']) {
                        ...
                        while (true) {
                            ...
                            $responseObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $optionsResp);
        ...
        $responseObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $optionsFlight);
        $response = json_decode($responseObj->getContent(), true);
        ...
        $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, $optionsFlight, $responseObj->getContent(), $responseObj->getStatusCode());
    ...
    public function apiDoBooking(BookHotelFilter $filter): array
    {
        ...
        foreach ($responseData as $alternative) {
            ...
            if ($departureFlight == $filter->Items->first()->Offer_departureFlightId &&
        ...
        $responseObjInit = $httpClient->request(HttpClient::METHOD_POST, $url, $options);
        
        $this->showRequest(HttpClient::METHOD_POST, $url, $options, $responseObjInit->getContent(false), $responseObjInit->getStatusCode());
        $tempReservation = json_decode($responseObjInit->getContent(false), true);
        ...
        if (isset($tempReservation['type']) && $tempReservation['type'] === 'Unauthorized') {
            ...
            $this->showRequest(HttpClient::METHOD_POST, $url, $options, $responseObj->getContent(false), $responseObj->getStatusCode());
            $tempReservation = json_decode($responseObj->getContent(), true);
        }
        ...
        $options['body'] = json_encode($bodyUpdate); 

        // update reservation
        $url = $this->bookingUrl . '/Reservation/General/UpdateReservation';
        $responseObjUpdate = $httpClient->request(HttpClient::METHOD_POST, $url, $options);

        $this->showRequest(HttpClient::METHOD_POST, $url, $options, $responseObjUpdate->getContent(false), $responseObjUpdate->getStatusCode());
        $responseDataUpdate = json_decode($responseObjUpdate->getContent(false), true);
        ...
        if (isset($responseDataUpdate['type']) && $responseDataUpdate['type'] === 'Unauthorized') {
            ...
            $this->showRequest(HttpClient::METHOD_POST, $url, $options, $responseObj->getContent(false), $responseObj->getStatusCode());
            $responseDataUpdate = json_decode($responseObj->getContent(), true);
        }

        $options['body'] = json_encode($responseDataUpdate);

        $url = $this->bookingUrl . '/Reservation/General/CommitReservation';
        $responseObjCommit = $httpClient->request(HttpClient::METHOD_POST, $url, $options);

        $this->showRequest(HttpClient::METHOD_POST, $url, $options, $responseObjCommit->getContent(false), $responseObjCommit->getStatusCode());
        $responseData = json_decode($responseObjCommit->getContent(false), true);
        ...
        return [$booking, $responseObjCommit->getContent(false)];
    }
    ...
    public function apiTestConnection(): bool
    {
        ...
        $responseObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $options);
        $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, $options, $responseObj->getContent(false), $responseObj->getStatusCode());
        $responseGeo = json_decode($responseObj->getContent(false), true);
        $geo = true;
        if ($responseGeo['Error'] && $responseGeo['Response'] === 'ERROR_SESSION_NOT_FOUND') {
            ...
            $responseObj = $httpClient->request(HttpClient::METHOD_POST, $this->apiUrl, $options);
            $this->showRequest(HttpClient::METHOD_POST, $this->apiUrl, $options, $responseObj->getContent(false), $responseObj->getStatusCode());
            $responseGeo = json_decode($responseObj->getContent(), true);
        }
        ...
        $responseObj = $httpClient->request(HttpClient::METHOD_POST, $url, $optionsLogin);
        $this->showRequest(HttpClient::METHOD_POST, $url, $optionsLogin, $responseObj->getContent(false), $responseObj->getStatusCode());
        $responseBook = json_decode($responseObj->getContent(false), true);
