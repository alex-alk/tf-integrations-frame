...
/**
 * Description of TravelioKarpatenStorage
 *
 * @author Omi-Mihai
 */
class TravelioKarpatenStorage extends \QStorageEntry implements \QIStorage
{
	...
	public static $Config = [
		'travelio_karpaten' => [
			...
			"duration_add" => 8,
			'reqs_on_city' => true,
			'skip_cacheTopHotels_onCitiesRequests' => false,
			'get_hotels_only_for_active_destinations' => true,
			'get_hotels_extra_details' => true
			#'filter_geo_and_hotels_with_active_in_sync' => true
		]
	];

	public static $UseRegionsInIndividualRequests = false;

	public $expandChartersTransports = false;

	public function __construct($name = null, \QStorageFolder $parent = null) 
	{
		$this->TOPInstance = new TravelioKarpaten();
	...
	public function initTourOperator($config = [])
	{
		...
		// cache top tours
		$this->cacheTOPTours($config, true);

		// charter departures
		$this->cacheChartersDepartures($config, true);

		// cache tours departure
		$this->cacheToursDepartures($config, true);
	}
	...
	public function saveTours($config = [])
	{
		$this->cacheTOPTours($config);
		return $this->cacheToursDepartures($config);
	}
	...
	public static function ApiQuery($storage_model, $from, $from_type, $selector = null, $parameters = null, $only_first = false, $id = null)
	{
		...
		switch ($from)
		{
			...
			case "Circuits" : 
			{
				...
				return $self->getToursOffers($parameters, $initialParams);
			}
			case "TourFees" : 
			{
				return $self->getOfferCancelFees($parameters, $initialParams);
