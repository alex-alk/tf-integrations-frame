...
class InfiniteApiService extends AbstractApiService
{
    ...
    public function apiTestConnection(): bool
    {
        ...
        try {
            ...
            if (count($response->Body->getCountriesResponse->country) > 0) {
                return true;
            }
        } catch (Throwable $e) {
            return false;
            ...
            foreach ($requests as $request) {
                ...
                if ($responseObj->getStatusCode() !== 200) {
                    while(true) {
                        ...
                        $content = $responseObj->getContent(false);
                        $this->showRequest(HttpClient::METHOD_POST, $url, $options, $content, $responseObj->getStatusCode());
