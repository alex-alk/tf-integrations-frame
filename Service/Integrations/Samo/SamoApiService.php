...
class SamoApiService extends AbstractApiService
{
	...
	*/
	...
	public function apiGetRegions(): RegionCollection
	{
		...
		if ($json === null) {
			...
			$regionsResp = $this->getDataFromXmlGate('region');

			foreach ($regionsResp as $regionResp) {

				$country = $countries->get((string) $regionResp['state']);
				$region = Region::create($regionResp['inc'], $regionResp['lname'], $country);
				$regions->put($region->Id, $region);
			}
			Utils::writeToCache($this, $file, json_encode($regions));
		} else {
			$regions = ResponseConverter::convertToCollection(json_decode($json, true), RegionCollection::class);
		}
	...
	private function requestData(string $action, array $params = []): array
	{
		...
		if (in_array($action, $cachedActions)) {
			...
			if ($json === null) {
				...
				if ($this->handle === Handles::ANEX_V2) {
	...
	{
		SamoValidator::make()->validateUsernameAndPassword($this->post)
	...
	{
		SamoValidator::make()->validateAllCredentials($this->post)
	...
	public function getAvailabilityDates(AvailabilityDatesFilter $filter): AvailabilityDatesCollection
	{
		...
		foreach ($townFroms as $townFrom) {
			...
			if (($this->handle === self::JOIN_UP || $this->handle === self::JOIN_UP_TEST)) {
				if ($townFrom['stateFromKey'] !== self::JOIN_UP_ROMANIA) {
					continue;
				}
			} elseif ($this->handle === Handles::PRESTIGE_V2 || $this->handle === Handles::PRESTIGE_TEST) {
				if ($townFrom['stateFromKey'] !== self::PRESTIGE_ROMANIA) {
					continue;
				}
			} else {
				if ($townFrom['stateFromKey'] !== self::ANEX_ROMANIA) {
					continue;
				}
			}
			...
			foreach ($states as $state) {
				...
				foreach ($toursResp as $tour) {
					...
					if ($filter->type === AvailabilityFilter::SERVICE_TYPE_CHARTER && $tour['typeKey'] !== self::TYPE_CHARTER) {
						continue;
					}


					if ($filter->type === AvailabilityFilter::SERVICE_TYPE_TOUR) {
						if ($tour['typeKey'] !== 3 && $tour['typeKey'] !== 15) {
							continue;
						}
					}
					...
					if (!$filter->onlyIds) {
						...
						foreach ($holidayCheckinsData['items'] as $holidayCheckin) {
							...
							if (!empty($holidayCheckin) && !empty($holidayNightsData)) {
								$dates[] = [
					...
					foreach ($holidayCitiesData as $holidayCity) {
						...
						$city = $cities->get($townFrom['id']);

						$transportCityFrom->City = $city;

						$cityTo = $cities->get($holidayCity['id']);

						$transportCityTo = new TransportCity();
						$transportCityTo->City = $cityTo;

						$transportId = AvailabilityDates::TRANSPORT_TYPE_PLANE . "~city|" . $city->Id . "~city|" . $cityTo->Id;

						$existingTransport = $avDates->get($transportId);

						$transport = new AvailabilityDates();
						$datesObj = new TransportDateCollection();

						if ($existingTransport !== null) {
							if (!$filter->onlyIds) {
								$datesObj = $existingTransport->Dates;
							}
							$transport = $existingTransport;
						} else {
							$transport->TransportType = AvailabilityDates::TRANSPORT_TYPE_PLANE;
							$transport->Id = $transportId;
							$transport->From = $transportCityFrom;
							$transport->To = $transportCityTo;
						}
						...
						if (!$filter->onlyIds) {
							foreach ($dates as $checkinData) {
								...
								$dateObj->Date = (new DateTime($checkin))->format('Y-m-d');
								$nights = new DateNightCollection();

								foreach ($checkinData['nights']['nights'] as $nightInt) {

									$nightsObj = new DateNight();
									$nightsObj->Nights = $nightInt;
									$nights->put($nightInt, $nightsObj);
								}

								$dateObj->Nights = $nights;
								$datesObj->put($dateObj->Date, $dateObj);
							}
							$transport->Dates = $datesObj;
						}

						$avDates->put($transport->Id, $transport);
					}
				}
			}

			if (!empty($filter->Id)) {
				$avDates = new AvailabilityDatesCollection([$avDates->get($filter->Id)]);
			}
		}

		//dd($avDates);
		// get the cities from the same dep city -> region
		// de verificat daca orasele de plecare
		// $cityRegions = [];

		// /** @var AvailabilityDates $avDate */
		// foreach ($avDates as $avDate) {
		// 	$dates = '';
		// 	/** @var TransportDate $date */
		// 	foreach ($avDate->Dates as $date) {
		// 		$dates .= $date->Date . '-';
		// 	}
		// 	if (isset($cityRegions[$avDate->From->City->Id . '-'.$avDate->To->City->County->Id])) {
		// 		if ($cityRegions[$avDate->From->City->Id . '-'.$avDate->To->City->County->Id][0] !== $dates) {
		// 			dump($avDate, $cityRegions[$avDate->From->City->Id . '-'.$avDate->To->City->County->Id][1]);
		// 		}
		// 	}
		// 	$cityRegions[$avDate->From->City->Id . '-'.$avDate->To->City->County->Id] = [$dates, $avDate];
		// }
