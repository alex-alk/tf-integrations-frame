...
use Omi\TF\TOInterface;
use Omi\TF\TOInterface_Util;
use Utils\Utils;

class GoGlobal extends TOInterface
{
	// ------added---------
	...
	public function api_getHotels(array $filter = null): array
	{
		$cacheFolder = $this->getResourcesDir();

		// get cache file
		$hotels = $cacheFolder . 'hotel-list.csv';

		//$filePointer = fopen($hotels, 'w+');
		//$url = 'https://static-data.tourismcloudservice.com/agency/hotels/' . ($this->ApiContext__ ?: $this->ApiContext);
		//$this->downloadFile2($url, $filePointer);

		// $hotelsArr = [];
		// if (($handle = fopen($hotels, "r")) !== FALSE) {
		...
		// $oldHotels = $this->api_getHotelss();

		// $i = 0;
		// foreach ($hotelsArr as $k => $h) {
		// 	if (!isset($oldHotels[$k])) {
		// 		$i++;
		// 		dump($h);
		// 	}
		// }
		...
		$url = 'https://static-data.tourismcloudservice.com/agency/hotels/' . ($this->ApiContext__ ?: $this->ApiContext);
		dump($url);
		$token = base64_encode($this->ApiUsername . ':' . $this->ApiPassword);
		dump('Authorization: Basic ' . $token);
...
*/
	 ...
	 *		array of: Id,Name,Code
	 */
	public function api_getHotelss(array $filter = null)
	{

		$file = 'hotel-list-old';
		$hotelListOld = Utils::getFromCache($this->TourOperatorRecord->Handle, $file);
		if ($hotelListOld === null) {
			...
			$citiesr = $this->api_getCities()[0];
			$hotels = [];
			$i = 0;
			foreach ($citiesr as $city) {
				$i++;
				Log::debug($i);
			...
			\Omi\TF\TOInterface::markReportEndpoint($filter, 'hotels');
			Utils::writeToCache($this->TourOperatorRecord->Handle, $file, json_encode($hotels));
		} else {
			$hotels = json_decode($hotelListOld, true);
		}
		// return hotels
		return $hotels;
	}
	...
	public function downloadFile2($url, $filePointer)
	{
		// init curl
		$ch = q_curl_init_with_log();
		//$fp = fopen('goglobal_curl_errorlog.txt', 'w');
	...
	public function request($method, $operation, $requestString = null, $useCache = false, $logData = false)
	{
		...
		if (!$response)
		{
			try {
				...
				// init soap client
				$soapClient = new \Omi\Util\GoGlobalSoap(($this->ApiUrl__ ?: $this->ApiUrl), $params);
