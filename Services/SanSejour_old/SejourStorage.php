 ...
 * @author Omi-Mihai
 */
class SejourStorage extends \QStorageEntry implements \QIStorage
{
	...
	public function initTourOperator($config = [])
	{
		...
		//q_die("after caching countries!");

		// cache top regions
		$this->cacheTOPRegions($config);
		//q_die("after caching regions!");

		// cache top cities
		$this->cacheTOPCities($config);
		//q_die("after caching cities!");

		$this->cacheTOPHotels(array_merge($config, ['get_extra_details' => true]), false, ["get_extra_details" => true]);
		//q_die("after caching hotels!");

		// setup on individual all
		$this->setupOnIndividualAll();
		#$this->cacheTOPHotelsDetails($config, false);
		#q_die("after caching hotels - details!");
	}
