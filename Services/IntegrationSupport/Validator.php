...
class Validator
{
    ...
    public function validateIndividualOffersFilter(array $post): self
    {
        ...
        if ((int) $post['args'][0]['rooms'][0]['children'] > 0 
            && count($post['args'][0]['rooms'][0]['childrenAges']) !== (int) $post['args'][0]['rooms'][0]['children']) {
                throw new Exception('childrenAges count is not correct');
        }
        ...
        if (empty($post['args'][0]['travelItemId']) && empty($post['args'][0]['cityId']) && empty($post['args'][0]['regionId'])) {
